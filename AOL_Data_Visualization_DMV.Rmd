...
dfpolut
```
I only need the 2021 year data as all of the data are from 2021 (outdated perhaps, but still SDG)
```{r}
subsetcar <- dfcar[c(1:4),c(1,3)]
subsetcar
```
```{r}
...
```
```{r}
 plot_ly(totalgreen, x = ~Kota.Adm, y = ~Total_Jumlah, type = 'bar', name = 'Total Jumlah', marker = list(color = 'blue')) %>%
  layout(
    title = "Total Jumlah by Kota.Adm",
    xaxis = list(title = "Kota.Adm"),
    yaxis = list(title = "Count"),s
    barmode = 'group'
  )
```
```{r}
plot_ly(totalgreen, x = ~Kota.Adm, y = ~Total_Luas, type = 'bar', name = 'Total Luas', marker = list(color = 'red')) %>%
  layout(
    title = "Total Luas by Kota.Adm",
    xaxis = list(title = "Kota.Adm"),
...
As the January one do not have data for PM25, i suppose we might just exclue January from our analysis
```{r}
...
```
Also try to find outlier and from the dot, certainly there are
```{r}
...
critpolut2
```
Here i use mutate function to change the date formatting, because i want to harvest the monthly information of each pollutant critical occurence, and turns out PM5 is still dominating
```{r}
subsetpolut <- subsetpolut %>%
...
```
```{r}
ggplot(totalgreen, aes(x = 2, y = Kota.Adm, fill = Total_jumlah)) + geom_bar(stat = "identity", width = 1) + coord_polar(theta = "y", start = 0) + geom_bar(stat = "identity", width = 0.5, aes(y=0)) + xlim(0.5, 2.5) + labs(fill = "Kabupaten") + theme_bw() + ggtitle("Tingkat kontribusi SPKU per daerah 2021")
```
...
ggplot(greenmonth, aes(y = jumlah, x = jenis, fill = location)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Location", y = "Count", title = "Occurrence of Green Area to Each Region in 2021") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
```{r}
ggplot(total, aes(y = jumlah, x = jenis, fill = location)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Location", y = "Count", title = "Occurrence of Green Area to Each Region in 2021") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
